...
;(function($, window, document, undefined) {	
	...
	var boxBg = 'rgba(26,32,34,.8)';  // line-94;
	methods = {
        init: function (options) {
           	return this.each(function() {
				var T = $(this), _O = T.data('hDialog');
				//*********************************************************alert(T.attr("linan_w"));
                if(typeof(_O) == 'undefined') {
	                	var defaults = {
	                		title: '',              	    //弹框标题
							box: '#HBox',               //弹框默认选择器
						//	boxBg: '#ffffff',           //弹框默认背景颜色
							modalBg: 'rgba(0,0,0,0.2)', //遮罩默认背景颜色
        ...
        open: function (o,T) {
			...
			//关闭按钮
			if(o.closeHide != false) closeBtnTpl = '<a id="HCloseBtn" title="关闭" style="width:50px;height:24px;line-height:24px;display:inline-block;cursor:pointer;color:#fff;font-size:1.4em;text-align:center;position:absolute;top:18px;right:15px;"><span style="width:50px;height:24px;display:inline-block;font-size: 14px;color: #A0DDD5;">关闭</span></a>';
			...
			if(o.types == 2){ iframeTpl = '<iframe width="' + o.width + '" height="' + o.height + '" frameborder="0" scrolling="auto" src="' + o.iframeSrc + '"></iframe>'; $B.append('<div id="'+o.iframeId+'"></div>'); $obj = $('#'+o.iframeId); }
        		...
        		$B.stop().append(overlayTpl).find($obj).css({'background-color':boxBg,'position':'fixed','top':t,'left':l,'z-index':999999,'margin':m,'width':o.width,'height':o.height}).removeAttr('class').addClass('animated '+c).prepend(headTpl+closeBtnTpl+iframeTpl).show();
