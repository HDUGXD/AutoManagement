...
function ButtonCapturePictureByWndNo_onclick()
{
	...
	alert("存储路径："+path);
	//alert("gWndId="+gWndId+"  "+"nWndNo="+nWndNo+"  "+"hWnd="+hWnd);
...
function ButtonCapturePictureByHWND_onclick()
{
	...
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
	var hWnd = obj.DPSDK_GetWndVideoHandle(0, 0);
	
	var mydate=new Date();
	var reg=new RegExp(":","g");
	var path="c:\\dahuaphoto\\"+mydate.toLocaleString().replace(" ","").replace("年","").replace("月","").replace("日","").replace(reg,"")+".bmp";
	//alert("存储路径："+path + ",gWndId=" + gWndId + ",nWndNo=" + nWndNo + ",hWnd=" + hWnd);
	alert(obj.DPSDK_CapturePictureByHWND(136150, path));
	//ShowCallRetInfo(obj.DPSDK_CapturePictureByHWND(136150, path), "抓图");
}
...
function showErrorInfo(errorCode){
	$.ajax({
		url: ROOT +"/screen/getcameraerrorinfo",
		type: "POST",
		data: "code=" + errorCode,
