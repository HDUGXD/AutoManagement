(function(f, define){
  define([ "../framework/js/kendo.list.min", "../framework/js/kendo.mobile.scroller.min" ], f);
})(function(){
  ...
  (function($, undefined) {
    ...
    var ComplexeSelect = List.extend({
      ...
      add : function(key,name){
        ...
        // that.dataSource.fetch();
        var value = [key];
        var dataItemIndex = that._index(value[0]);
        if (dataItemIndex > -1) {
          that._select(dataItemIndex);
        }
      },
      ...
      _tagList: function() {
        ...
        if (!tagList[0]) {
          tagList = $('<ul role="listbox" unselectable="on" class="k-reset"></ul>').appendTo(that._innerWrapper);
        }
