...
function quick_dialog(id,title,url,width,height,btn,reload){
    ...
    if(height>=document.body.scrollHeight-100)
	{
		height=document.body.scrollHeight-100;
	}
	if(width>=document.body.scrollWidth-100)
	{
 ...
 * 参数(reload)：Boolean，指定是否在弹出框弹出时刷新iframe页面，可选
 **/
function quick_dialog_img(id,title,url,width,height,btn,reload){
	if(window != top.window){
		top.quick_dialog_img(id,title,url,width,height,btn);
		return;
	}
	if(height>=document.body.scrollHeight-100)
	{
 ...
 * 参数(toolbarId)：工具条的id，可选
 **/
function quick_datagrid_no_input(el,formId,readUrl,columns,options,dsOptions,operates,toolbarId){
	options = options || {};
	toolbarId  = toolbarId || "#grid_toolbar_template";
	var _dbopt = {
		transport: {
 ...
 * 参数(toolbarId)：工具条的id，可选
 **/
function quick_datagrid_no(el,formId,readUrl,columns,options,dsOptions,operates,toolbarId){
    options = options || {};
    toolbarId  = toolbarId || "#grid_toolbar_template";
    var _dbopt = {
        transport: {
...
function btn_running(btn,text) {
	...
	if (btn.tagName.toUpperCase() == "A") {
        ...
        var button = $(btn).hcButton().data("kendoButton");
		if (button) {
